...
이 분야에서 많은 기여를 한 **다이시 카토(Dai Shi)**의 트위터와 저서 등을 참고해, 전역 상태의 개념과 전역상태관리를 실현하기 위한 방법들을 실제 코드로 구현하며 동작원리 중심으로 리액트 생태계에서 상태 관리를 어떻게 바라보고 활용할 수 있을지 내 관점을 다시 한번 정리해 보았다. 
