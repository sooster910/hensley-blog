...
당시 서비스는 빠른 기능 출시를 우선시하는 과정에서 REST 원칙과 문서화가 충분히 이뤄지지 않았고, 응답 구조 역시 기능별로 상이했다.

**이를 표준화 부재의 문제로 정의하고 개선 방향을 모색했다**
...
1. **Relay**. (n.d.). *Relay: A JavaScript framework for building data-driven React applications*. Retrieved from https://relay.dev/